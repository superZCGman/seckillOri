...
(function() {
	...
	var $ImageUpload = function(pictureId,muti,module) {
		this.pictureId = pictureId;
		this.muti = muti;
		this.uploadBtnId = pictureId + "BtnId";
		this.uploadPreId = pictureId + "PreId";
		this.uploadDivId = pictureId + "DivId";
		this.uploadUrl = Feng.ctxPath + '/image/upload?module='+module;
	...
	$ImageUpload.prototype = {
		...
		bindEvent : function(bindedObj) {
			...
			bindedObj.on('uploadSuccess', function(file,response) {
				Feng.success("上传成功");
				if(me.muti) {
					$("#"+me.uploadDivId).append("<input class='img-url' value='"+response+"' readonly='readonly'/><a class='img-url-delete'>删除</a><br>");
				} else {
					if($(".img-url").length>0) {
						$(".img-url").val(response);
					} else {
						$(".img-url-div").append("<input class='img-url' value='"+response+"' /><a class='img-url-delete'>删除</a><br>");
					}
				}
				
				var tempImgUrl = "";
				var length = $(".img-url").length;
				for(var i=0;i<length;i++) {
					if(tempImgUrl=="") {
						tempImgUrl += $($(".img-url")[i]).val();
					} else {
						
						tempImgUrl+="#"+$($(".img-url")[i]).val();
					}
				}
				$("#" + me.pictureId).val(tempImgUrl);
			});
		...
		imgUrlDelete : function() {
			var me = this;
			$("body").on("click",".img-url-delete",function() {
				$(this).prev().remove();
				$(this).next().remove();
				$(this).remove();
				var tempImgUrl = "";
